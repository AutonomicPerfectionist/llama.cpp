...
void begin_async_run(const llama_sampling_params& sparams, int n_seq_dft,
                     ...
                     size_t &batch_id, int &n_past, llama_kv_cache_view &kvc_view,
...
bool start_async_spec_run(const gpt_params &params, llama_context *ctx_tgt, llama_context *ctx_dft,
                          ...
                          int &spec_past_tgt, int &spec_past_dft, int first_run, int orig_offset, size_t &batch_id,
...
void begin_non_spec_run(const gpt_params &params, int n_seq_dft, llama_context *ctx, int max_seq,
                        const std::vector<seq_draft> &drafts, llama_token id, size_t &batch_id, int &n_past, int n_past_dft,
...
run_speculation_loop(const gpt_params &params, const float p_accept, llama_context *ctx_tgt, llama_context *ctx_dft,
                     ...
                     std::deque<int> &free_sequence_offsets, size_t &batch_id, llama_batch &batch_dft, int &n_drafted,
...
//    printf("Size of first split: %lu, element: %f\n", params.mpi_layer_split[0].size(), params.mpi_layer_split[0][0]);
    ...
    std::tie(model_dft, ctx_dft) = llama_init_from_gpt_params(params, true);

#ifndef LOG_DISABLE_LOGS
    llama_switch_partition(ctx_tgt, MPI_PARTITION_WORLD);
    log_set_target(log_filename_generator("pipeinfer-node-" + std::to_string(llama_node_id(ctx_tgt)), "log"));
...
//    llama_pop_request(ctx_tgt);
    ...
    while (true) {
        ...
        bool is_waiting = llama_mpi_test_recv(ctx_tgt);
//        bool is_waiting = true;
        llama_switch_partition(ctx_tgt, MPI_PARTITION_ROOTS);
        llama_sync_bool(ctx_tgt, &is_waiting, 0);
        llama_switch_partition(ctx_tgt, MPI_PARTITION_MODEL);
        ...
        if (!tgt_cgraphs.empty() && is_waiting) {
            ...
            LOG("Finishing async decode, is spec = %d, old seq_offset = %d, new seq offset = %d, batch id = %zu\n", run.speculative, current_run.seq_offset, run.seq_offset, run.batch.batch_id);
...
//            LOG("Beginning sampling, dft cache layout:\n%s", dump_kv_cache_view_seqs(kvc_view_dft, 1).c_str());
        ...
        while (!keeps.empty()) {
            ...
            llama_sync_int32_t(ctx_tgt, &id, 0);
...
//            }
        ...
        is_waiting = llama_mpi_test_recv(ctx_tgt);
        llama_switch_partition(ctx_tgt, MPI_PARTITION_ROOTS);
        llama_sync_bool(ctx_tgt, &is_waiting, 0);
        llama_switch_partition(ctx_tgt, MPI_PARTITION_MODEL);
        first_run = false;
...
void begin_non_spec_run(const gpt_params &params, const int n_seq_dft, llama_context *ctx, const int max_seq,
                        const std::vector<seq_draft> &drafts, llama_token id, size_t &batch_id, int &n_past,
                        int n_past_dft,
                        std::deque<struct seq_async_run> &dft_cgraphs, llama_kv_cache_view &kvc_view) {
...
void begin_async_run(const llama_sampling_params& sparams, const int n_seq_dft,
    ...
    LOG("Beginning async decode, batch id = %zu\n", batch_id);
...
//            }
    ...
    llama_decode(ctx_tgt, run.batch);
